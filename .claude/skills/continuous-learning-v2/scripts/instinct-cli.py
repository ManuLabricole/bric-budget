...
def _validate_import_url(source: str) -> str:
    ...
    try:
        ...
        raise ValueError(
            f"remote import host could not be resolved: {parsed.hostname}"
        ) from exc
    ...
    for family, _, _, _, sockaddr in addr_infos:
        ...
        if (
            ...
            raise ValueError(
                f"remote import host resolves to a non-public address: {host}"
            )
...
def detect_project() -> dict:
    ...
    fallback_root = (
        _main_worktree_root(project_root) if not remote_url else project_root
    )
    ...
    hash_source = (
        ...
        else (remote_url if remote_url else fallback_root)
    )
...
def _merge_instinct_dir(
    ...
    for file_path in sorted(from_dir.iterdir()):
        if (
            ...
            or file_path.suffix.lower() not in ALLOWED_INSTINCT_EXTENSIONS
        ):
            continue
        try:
        ...
        if target_path.exists():
            target_path = (
                into_dir
                / f"{file_path.stem}-{_project_hash(str(file_path))}{file_path.suffix}"
            )
...
def _warn_legacy_data() -> None:
    ...
    try:
        ...
        print(
            f"\n  Note: legacy directory exists but cannot be read: {legacy_dir}",
            file=sys.stderr,
        )
        return
    if not legacy_files:
...
def _cmd_projects_delete(args) -> int:
    ...
    print(
        f"  Instincts: {counts['personal']} personal, {counts['inherited']} inherited"
    )
...
def _cmd_projects_gc(args) -> int:
    ...
    candidates = [
        ...
        if _validate_project_id(project_id)
        and _project_counts(project_id)["total"] == 0
    ]
...
def _cmd_projects_merge(args) -> int:
    ...
    print(
        f"  Source: {from_counts['personal']} personal, {from_counts['inherited']} inherited, {from_counts['observations']} observations"
    )
    print(
        f"  Destination before merge: {into_counts['personal']} personal, {into_counts['inherited']} inherited, {into_counts['observations']} observations"
    )
...
def main() -> int:
    ...
    projects_delete = projects_subparsers.add_parser(
        "delete", help="Delete a project registry entry"
    )
    ...
    projects_merge = projects_subparsers.add_parser(
        "merge", help="Merge one project registry entry into another"
    )
    ...
    projects_merge.add_argument(
        "--dry-run", action="store_true", help="Preview without merging"
    )
    projects_merge.add_argument(
        "--force", action="store_true", help="Skip confirmation"
    )
    projects_gc = projects_subparsers.add_parser(
        "gc", help="Delete zero-value project registry entries"
    )
    projects_gc.add_argument(
        "--dry-run", action="store_true", help="Preview without deleting"
    )
